...
std::atomic<bool> gimbal_data_ready(false);

static void processHandeyeImage(const CameraFrame& frame) {
...
// 串口数据处理回调函数
void gimbalDataCallback(const ReceivedDataMsg& data) {
...
public:
    HandEyeCalibrator(const std::string& config_path) 
        ...
        // 创建保存图像的目录
        image_save_path_ = "handeye_calibration_images";
        std::filesystem::create_directories(image_save_path_);
        
        ULOG_INFO_TAG("HandEyeCalibrator", "HandEyeCalibrator initialized with board size %dx%d", 
...
private:
    void captureImages() {
        ...
        subscriber.subscribe<CameraFrame>("camera/image", [](const CameraFrame& frame) {
            processHandeyeImage(frame);
        }, DeliveryMode::COPY);
        
        // 订阅云台数据
        subscriber.subscribe<ReceivedDataMsg>("serial/data", gimbalDataCallback, DeliveryMode::COPY);
        
        ULOG_INFO_TAG("HandEyeCalibrator", "Started capturing chessboard images");
        ...
        while (capturing_ && image_count_ < max_images_ && key != 27 && key != 'q' && key != 'Q') {
            ...
            if (key == 'c' || key == 'C') {
                ...
                if (pattern_found && image_count_ < max_images_ && gimbal_data_ready) {
                    ...
                    image_count_++;
                    ULOG_INFO_TAG("HandEyeCalibrator", "Captured image %d/%d with gimbal data", static_cast<int>(image_count_), max_images_);
                } else if (!pattern_found) {
                    ULOG_INFO_TAG("HandEyeCalibrator", "未检测到棋盘格，无法捕获图像");
                } else if (!gimbal_data_ready) {
                    ULOG_INFO_TAG("HandEyeCalibrator", "未收到云台数据，无法捕获图像");
                }
            }
        }
        ...
        ULOG_INFO_TAG("HandEyeCalibrator", "图像捕获完成，共捕获 %d张图像", static_cast<int>(image_count_));
    }
    ...
    void processFrame(const cv::Mat& frame) {
        ...
        cv::imshow("HandEye Calibration", display_frame);
    }
    ...
    bool calibrate() {
        ULOG_INFO_TAG("HandEyeCalibrator", "开始执行手眼标定...");
        ...
        for (const auto& entry : std::filesystem::directory_iterator(image_save_path_)) {
            if (entry.path().extension() == ".jpg") {
                ...
                if (found && data_exists) {
                    ...
                    valid_image_count++;
                    ULOG_INFO_TAG("HandEyeCalibrator", "Processed image %s with gimbal data", 
        ...
        if (image_points.size() < 4) {
            ULOG_ERROR_TAG("HandEyeCalibrator", "Not enough valid images for calibration (%d < 4)", 
                          static_cast<int>(image_points.size()));
            return false;
        }
        
        ULOG_INFO_TAG("HandEyeCalibrator", "使用 %d 张图像进行标定", static_cast<int>(image_points.size()));
        ...
        for (size_t i = 0; i < gimbal_data_list.size(); i++) {
            ...
            t_gimbal2world_list.push_back((cv::Mat_<double>(3, 1) << 0, 0, 0)); // 平移设为0
        }
        ...
        // 保存标定结果到配置文件
        saveCalibrationResults(R_gimbal2imubody_data, R_camera2gimbal, t_camera2gimbal, ypr);
    ...
    void saveCalibrationResults(
        ...
        ULOG_INFO_TAG("HandEyeCalibrator", "Calibration results saved to %s", config_path_.c_str());
        ...
        std::cout << "\n相机同理想情况的偏角 (度):\n";
        std::cout << "yaw: " << std::fixed << std::setprecision(2) << ypr[0] << "°\n";
        std::cout << "pitch: " << std::fixed << std::setprecision(2) << ypr[1] << "°\n";
        std::cout << "roll: " << std::fixed << std::setprecision(2) << ypr[2] << "°\n";
    }
...
int runHandeyeCalibration() {
    ...
    ULOG_INFO_TAG("main", "HandEye calibration started");
    
    try {
        HandEyeCalibrator calibrator("config/camera_set.yaml");
    ...
    ULOG_INFO_TAG("main", "HandEye calibration program finished");
    ULOG_DEINIT();
    return 0;
}
