 ...
 * @LastEditTime: 2025-08-29 18:23:39
...
void Tracker::drawDebug(cv::Mat& bgr_img)
{
  cv::Mat img = bgr_img.clone();
  
  // 绘制跟踪状态
  cv::putText(img, "Tracker State: " + state_, cv::Point(10, 30), 
    ...
    if (state_ == "tracking" || state_ == "temp_lost") {
      ...
      if (!target_.name.empty()) {
        // 绘制目标名称
        cv::putText(img, "Target: " + target_.name, cv::Point(10, 60), 
                    cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
        
        // 绘制目标装甲板类型
        cv::putText(img, "Type: " + ARMOR_TYPES[target_.armor_type], cv::Point(10, 90), 
                    cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
        
        // 绘制目标优先级
        cv::putText(img, "Priority: " + std::to_string(target_.priority), cv::Point(10, 120), 
                    cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
        
        // 绘制是否收敛
        std::string converge_status = target_.convergened() ? "Converged" : "Not Converged";
        cv::putText(img, "Status: " + converge_status, cv::Point(10, 180), 
                    cv::FONT_HERSHEY_SIMPLEX, 0.7, 
                    target_.convergened() ? cv::Scalar(0, 255, 0) : cv::Scalar(0, 0, 255), 2);
        
        // 绘制是否发生跳跃
        std::string jump_status = target_.jumped ? "Jumped" : "Not Jumped";
        cv::putText(img, "Jump: " + jump_status, cv::Point(10, 210), 
                    cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
      ...
      for (size_t i = 0; i < armor_xyza_list.size(); ++i) {
        ...
        if (pose_estimator_) {
          ...
          for (size_t j = 0; j < image_points.size(); ++j) {
            cv::line(img, image_points[j], image_points[(j+1) % image_points.size()], 
            cv::Scalar(0, 255, 0), 2);
          }
          
          // 标记装甲板ID
          if (!image_points.empty()) {
            cv::putText(img, std::to_string(i), image_points[0], 
    ...
    cv::putText(img, "Searching target...", cv::Point(10, 60), 
                cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255, 255, 0), 2);
  } else {
    // 当没有追踪目标时，显示提示信息
    cv::putText(img, "No target tracked", cv::Point(10, 60), 
                cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 255), 2);
  }
  // 创建缩小版图像用于显示
  cv::Mat resized_img;
  cv::resize(img, resized_img, cv::Size(bgr_img.cols/2, bgr_img.rows/2));
